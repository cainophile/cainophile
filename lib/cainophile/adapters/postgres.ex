defmodule Cainophile.Adapters.Postgres do
  ...
  def init(config) do
    ...
    publication_names = Keyword.get(config, :publications)
                        |> Enum.map(fn pub -> ~s("#{pub}") end)
                        |> Enum.join(",")

    case :epgsql.connect(epgsql_config) do
      {:ok, epgsql_pid} ->
        {:ok, slot_name} = create_replication_slot(epgsql_pid, Keyword.get(config, :slot, :temporary))
  ...
  defp create_replication_slot(epgsql_pid, slot) do
    {slot_name, start_replication_command} = case slot do
      name when is_binary(name) ->
        {name, "SELECT 1;"} # TODO
      :temporary ->
        slot_name = self_as_slot_name()
        {slot_name, "CREATE_REPLICATION_SLOT #{slot_name} TEMPORARY LOGICAL pgoutput NOEXPORT_SNAPSHOT"}
    end
  ...
  defp self_as_slot_name() do
    ...
    pid_number = String.replace(pid, ".", "_")
    |> String.slice(0..-2)
